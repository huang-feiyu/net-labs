...
char *print_mac(uint8_t *mac) {
  ...
  if (mac == 0) {
    ...
    sprintf(result, "%02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1], mac[2],
            mac[3], mac[4], mac[5]);
    return result;
  }
}
...
int check_log() {
  ...
  while (get_round(demo_log) == 0) {
    ...
    if ((ret = check_round())) {
      printf(
          "\e[0;31mRound %d: differences found(Line %d of the current round)\n",
          i, ret);
      result = 1;
    } else {
...
CHECK_PCAP_NEXT_PACKET:
  ...
  if (ret1 == -1) {
    fprintf(stderr,
            "\e[1;31mError occured on loading packet %d from user output:%s\n",
